...
CREATE TABLE employee (
   employee_id INTEGER AUTO_INCREMENT,      -- 회원번호
   department_id INTEGER NOT NULL,   -- 부서번호
   name VARCHAR(20) NOT NULL,      -- 사원명
   member_id VARCHAR(30) NOT NULL,   -- 사원번호
   password VARCHAR(100) NOT NULL,   -- 비밀번호
   email VARCHAR(30) NOT NULL,       -- 이메일
   phone VARCHAR(13),            -- 전화번호
   authkey varchar(50),          -- 인증번호
   state VARCHAR(13),               -- 인증상태
   authkey_date DATETIME DEFAULT NOW(),   -- 인증신청일시
